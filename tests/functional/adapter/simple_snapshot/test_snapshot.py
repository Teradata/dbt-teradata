...
class BaseSimpleSnapshotBase:
    ...
    def _assert_results(
        ...
        """
        records = set(self.get_snapshot_records("id, CASE WHEN dbt_valid_to IS NULL THEN 1 ELSE 0 END AS is_current"))
...
class BaseSimpleSnapshot(BaseSimpleSnapshotBase):
    ...
    #     """
    #     self.add_fact_column("full_name", "varchar(64000) default null")
    #     self.update_fact_records(
    #         {
...
class TestSnapshotTeradata(BaseSimpleSnapshot):
    pass

class TestSnapshotCheckTeradata(BaseSnapshotCheck):
    pass
